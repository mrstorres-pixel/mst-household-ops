  ...
  return (
    <>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <section className="card table-wrap"><table><thead><tr><th>Suppliers</th><th>Contact</th></tr></thead><tbody>{results.suppliers.map((row) => <tr key={row.id}><td>{row.name}</td><td>{row.contact_name ?? row.phone}</td></tr>)}{!results.suppliers.length ? <tr><td colSpan={2}>No suppliers found.</td></tr> : null}</tbody></table></section>
        <section className="card table-wrap"><table><thead><tr><th>Items</th><th>SKU</th></tr></thead><tbody>{results.items.map((row) => <tr key={row.id}><td>{row.name}</td><td>{row.sku}</td></tr>)}{!results.items.length ? <tr><td colSpan={2}>No items found.</td></tr> : null}</tbody></table></section>
        ...
        <section className="card table-wrap"><table><thead><tr><th>Cheques</th><th>Customer</th><th>Status</th><th>Amount</th></tr></thead><tbody>{results.cheques.map((row) => <tr key={row.id}><td>{row.cheque_number}</td><td>{relationName(row.customers)}</td><td>{row.status}</td><td>{money(row.amount)}</td></tr>)}{!results.cheques.length ? <tr><td colSpan={4}>No cheques found.</td></tr> : null}</tbody></table></section>
