  ...
  return (
    <>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <section className="card table-wrap"><table><thead><tr><th>Customer Invoices</th><th>Customer</th><th>Total</th><th>Open</th></tr></thead><tbody>{results.invoices.map((row) => <tr key={row.id}><td><Link className="font-bold text-[color:var(--primary)]" href={`/invoices/${row.id}/print`}>{row.invoice_number}</Link></td><td>{relationName(row.customers)}</td><td>{money(row.total)}</td><td><Link className="font-bold text-[color:var(--primary)]" href={`/invoices/${row.id}/edit`}>Edit</Link></td></tr>)}{!results.invoices.length ? <tr><td colSpan={4}>No invoices found.</td></tr> : null}</tbody></table></section>
