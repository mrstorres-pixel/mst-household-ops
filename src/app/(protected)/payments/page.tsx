...
export default async function PaymentsPage() {
  const [customerRows, payments] = await Promise.all([listCustomerRows(), listPayments()]);
    ...
    <>
      ...
      <section className="grid gap-5 lg:grid-cols-[380px_1fr]">
        ...
        <div className="card table-wrap">
          <table>
            <thead><tr><th>Date</th><th>Customer</th><th>Method</th><th>Amount</th><th>Reference</th></tr></thead>
            <tbody>
              {payments.map((payment) => <tr key={payment.id}><td>{payment.payment_date}</td><td>{payment.customers?.name}</td><td>{payment.method}</td><td>{money(payment.amount)}</td><td>{payment.reference}</td></tr>)}
              {!payments.length ? <tr><td colSpan={5}>No payments yet.</td></tr> : null}
            </tbody>
          </table>
        </div>
      </section>
